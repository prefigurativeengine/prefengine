...
impl RemotePeerInfo {
    pub fn from_temps_write(temps: Vec<TempPeerInfo>) -> Result<(), String> {
        ...
        for t in temps {
            new_peers.push(Self::new(t.dest_hash)?);
        }
    ...
    pub fn append_peers_to_disk(new_peers: Vec<RemotePeerInfo>) -> Result<(), String> {
        ...
        match peer_path_res {
            Ok(peer_path) => {
                let peer_json = fs::read_to_string(&peer_path).map_err(|err| err.to_string())?;

                let mut json_array: Vec<RemotePeerInfo> =
                    serde_json::from_str(&peer_json).map_err(|err| err.to_string())?;
                json_array.extend(new_peers);

                let json_str = serde_json::to_string(&json_array).map_err(|err| err.to_string())?;
...
impl TempPeerInfo {
    pub fn append_temp_to_disk(temp: TempPeerInfo) -> Result<(), String> {
        let temp_json = fs::read_to_string("expected_temps.json").map_err(|err| err.to_string())?;

        let mut json_array: Vec<TempPeerInfo> =
            serde_json::from_str(&temp_json).map_err(|err| err.to_string())?;
    ...
    pub fn load_expected_temps() -> Result<Vec<TempPeerInfo>, String> {
        ...
        let temp_array: Value = serde_json::from_str(&temp_json).map_err(|err| err.to_string())?;

        let mut expected = vec![];

        for t in temp_array.as_array().unwrap() {
            let temp_obj: TempPeerInfo =
                serde_json::from_value(t.clone()).map_err(|err| err.to_string())?;
            expected.push(temp_obj);
        }
...
use serde_json::{Error as s_Error, Map, Number};
impl SelfPeerInfo {
    // TODO: clean this function up, and support ipv6
    pub fn new_self_peer(
        cap: PeerCapability,
        ip: Ipv4Addr,
        dest_hash: String,
    ) -> Result<SelfPeerInfo, String> {
        let self_dummy_path = Path::new("self_peer.dummy.json");
        let self_path = Path::new("self_peer.json");

        let dummy_str = fs::read_to_string(self_dummy_path).map_err(|err| err.to_string())?;

        let mut self_map: HashMap<String, Value> =
            serde_json::from_str(&dummy_str).map_err(|err| err.to_string())?;

        let id = RemotePeerInfo::get_next_unique_id()?;

        *self_map.get_mut("id").unwrap() = Value::Object(Map::from_iter([(
            "value".to_owned(),
        ...
        *self_map.get_mut("disk").unwrap() = json!({});
        ...
        let self_str = serde_json::to_string(&self_map).map_err(|err| err.to_string())?;
    ...
    fn get_self_peer_from_disk(peer_path: &str) -> Result<SelfPeerInfo, String> {
        ...
        let json_obj = serde_json::from_str(&peer_json).map_err(|err| err.to_string())?;
